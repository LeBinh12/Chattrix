...
func CompleteProfileHandler(db *mongo.Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if user.IsProfileComplete {
			...
			roles, err := business.GetUserRoles(c.Request.Context(), user.ID.Hex())
			if err != nil {
				roles = []string{}
			}
...
func UpdateProfileHandler(db *mongo.Database) gin.HandlerFunc {
	return func(c *gin.Context) {
		// Lấy userID từ JWT middleware
		userIDRaw, exists := c.Get("userID")
		if !exists {
		...
		// Tạo Biz và gọi CompleteProfile (reusing logic as it updates user)
		store := storage.NewMongoStore(db)
		business := biz.NewUserBiz(store)
		...
		// Lấy user sau khi update để trả về
		user, err := business.GetUserByID(c.Request.Context(), userIDStr)
		if err != nil {
			c.JSON(http.StatusInternalServerError, common.NewResponse(500, "Không tìm thấy user", nil))
			return
		...
		c.JSON(http.StatusOK, common.NewResponse(200, "Cập nhật hồ sơ thành công", user))
	}
}
